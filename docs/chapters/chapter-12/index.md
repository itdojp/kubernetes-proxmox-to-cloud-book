...
本番運用の品質は「手順が存在する」だけでは担保できません。検証環境で、次の “最小演習” を 1 回は実施し、手順と判断基準を更新してください。

- 監視/アラート: 意図的に Pod を `CrashLoopBackOff` にし、通知→一次切り分け→復旧までを通す（ノイズ設計も確認する）
- ログ: `kubectl logs` とノード側ログ（kubelet/containerd）を同じ事象に紐付けて収集できることを確認する
- RBAC: 日常権限（読み取り中心）と緊急権限（Break-glass）の運用手順を分け、監査ログに残ることを確認する
